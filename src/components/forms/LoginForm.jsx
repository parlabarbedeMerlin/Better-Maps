...
const RegisterForm = () => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("/api/auth/login", connection)
      .then(res => {
        setPopup(true)
  ...
  return (
    <>
      ...
      <form className="bg-white px-10 py-9 rounded-[35px] flex flex-col gap-6 items-center w-11/12 md-20 sm:my-0 md:max-w-96 md:w-11/12 sm:w-7/12 drop-shadow-2xl">
        ...
        <Button className={clsx(dosis.className)} type={"button"} variant="primary" onClick={handleSubmit}>
          Login
        </Button>
      </form>
    </>
