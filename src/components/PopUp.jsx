...
const PopUp = ({ open, onClose, clickBtn, message, success, btnText }) => (
  <Dialog open={open} onClose={onClose}>
    ...
    <div className={"fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 flex flex-col justify-center gap-2 w-11/12 md:w-1/4 h-1/4 items-center bg-white p-10 rounded-xl"}>
      <Dialog.Title className={clsx(dosis.className, "text-4xl font-bold")}>
        {success ? "✅ Success" : "❌ Error"}
      </Dialog.Title>
      <Dialog.Description className={clsx("text-lg text-center font-semibold", { "text-green-600": success, "text-red-500": !success })}>
        {success ? "You have been registered" : "An error occurred"} <br />
        {message}
      </Dialog.Description>
